...
from cipher_game import register_cipher_handlers, SESSIONS as cipher_sessions, load_data as load_cipher_data
from memo_game import register_memo_handlers, SESSIONS as memo_sessions, reload_data as reload_memo_data
from archie import register_archie_handlers, SESSIONS as archie_sessions, reload_data as reload_archie_data
from quiz_game import register_quiz_handlers, load_quizzes
...
RANK_REWARDS = {
    "🟢Опытный🟢":      {"money": 500, "bait": 1,  "eggs": 0},
    "🔵Профи🔵":        {"money": 600, "bait": 3,  "eggs": 0},
    "🟣Мастер🟣":       {"money": 1000, "bait": 5,  "eggs": 1},
    "🔴Грандмастер🔴":  {"money": 3500, "bait": 7,  "eggs": 3},
    "💠Легенда💠":      {"money": 7000, "bait": 9,  "eggs": 5},
    "🔱Император🔱":    {"money": 15000, "bait": 10, "eggs": 8},
}
...
def cmd_exit_game(message):
    ...
    # Выход из .архи
    if user_id in archie_sessions:
        del archie_sessions[user_id]
        exited = True
    
    # Выход из .мемо
    if user_id in memo_sessions:
        del memo_sessions[user_id]
        exited = True
    ...
    if exited:
        ...
        bot.send_message(message.chat.id,
            ...
            ".мемо — запомни узор\n"
            ".шифр — расшифруй слово\n"
            ".квиз — пройди викторину", parse_mode="Markdown")
...
def shop_keyboard():
    ...
    markup.add(
        ...
        InlineKeyboardButton("🥚 Загадочное Яицо ×1",          callback_data="buy_egg_1"),
        InlineKeyboardButton("👜🥚 Сумка яиц ×5",              callback_data="buy_egg_5"),
        InlineKeyboardButton("💼🥚 Чемодан яиц ×25",           callback_data="buy_egg_25"),
        InlineKeyboardButton("🛢️🥚 Бочка яиц ×50",            callback_data="buy_egg_50"),
        InlineKeyboardButton("🛻🥚 Багаж яиц ×200",            callback_data="buy_egg_200"),
        InlineKeyboardButton("🚢🥚 Крейсер яиц ×1000",         callback_data="buy_egg_1000"),
    )
    return markup
...
def cmd_bag(message):
    ...
    buttons = []
    if eggs     > 0:
        buttons.append(InlineKeyboardButton("🥚 Открыть ×1", callback_data="open_egg_bag"))
        if eggs >= 10:
            buttons.append(InlineKeyboardButton("🥚🥚 Открыть ×10", callback_data="open_egg_bag_10"))
...
def cmd_shop(message):
    ...
    bot.send_message(
        ...
        f"🥚 Яиц: {eggs}\n"
        f"{sep}\n"
        f"🥚 Загадочное Яицо ×1 — 💵 250\n"
        f"👜🥚 Сумка яиц ×5 — 💵 1,125\n"
        f"💼🥚 Чемодан яиц ×25 — 💵 5,625\n"
        f"🛢️🥚 Бочка яиц ×50 — 💵 11,250\n"
        f"🛻🥚 Багаж яиц ×200 — 💵 45,000\n"
        f"🚢🥚 Крейсер яиц ×1000 — 💵 225,000\n"
        f"{sep}",
...
EGG_SHOP_ITEMS = {
    ...
    "buy_egg_25":   (25,   5625),
    "buy_egg_50":   (50,   11250),
    "buy_egg_200":  (200,  45000),
    "buy_egg_1000": (1000, 225000),
}

EGG_SHOP_LABELS = {
    "buy_egg_1":    "🥚 Загадочное Яицо ×1",
    "buy_egg_5":    "👜🥚 Сумка яиц ×5",
    "buy_egg_25":   "💼🥚 Чемодан яиц ×25",
    "buy_egg_50":   "🛢️🥚 Бочка яиц ×50",
    "buy_egg_200":  "🛻🥚 Багаж яиц ×200",
    "buy_egg_1000": "🚢🥚 Крейсер яиц ×1000",
}

@bot.callback_query_handler(func=lambda call: call.data in EGG_SHOP_ITEMS)
def callback_buy_egg_bundle(call):
    user_id = call.from_user.id
    user = get_user(user_id)
    qty, price = EGG_SHOP_ITEMS[call.data]
    label = EGG_SHOP_LABELS[call.data]
    if not user or user[2] < price:
        bot.answer_callback_query(call.id, f"❌ Недостаточно средств! Нужно 💵{price:,}")
        return
    spend_money(user_id, price)
    add_eggs(user_id, qty)
    bot.answer_callback_query(call.id, f"✅ {qty} яиц добавлено в мешок!")
    bot.send_message(call.message.chat.id,
        f"✅ Куплено *{label}*!\nОткрой через */bag*", parse_mode='Markdown')
...
# --- Открытие 10 яиц из мешка ---
@bot.callback_query_handler(func=lambda call: call.data == 'open_egg_bag_10')
def callback_open_egg_bag_10(call):
    user_id = call.from_user.id
    user = get_user(user_id)
    eggs = user[20] if len(user) > 20 else 0
    if eggs < 10:
        bot.answer_callback_query(call.id, "❌ Нужно минимум 10 яиц!")
        return
    conn = get_conn()
    c = conn.cursor()
    c.execute('UPDATE users SET eggs=eggs-10 WHERE user_id=%s', (user_id,))
    conn.commit()
    conn.close()
    bot.answer_callback_query(call.id, "🥚 Открываем 10 яиц...")
    for i in range(10):
        msg = bot.send_message(call.message.chat.id,
            f"🥚❔ ЗАГАДОЧНОЕ ЯЙЦО ❔🥚 [{i+1}/10]\n\n_Открывается. . ._", parse_mode='Markdown')
