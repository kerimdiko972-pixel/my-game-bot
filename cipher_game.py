...
def _norm(word: str) -> str:
    """Нижний регистр + ё→е + только буквы."""
...
def _normalize(text: str) -> str:
    """Нижний регистр + ё→е + убрать всё кроме букв."""
    return text.lower().replace("ё", "е").strip()

def _normalize_word(word: str) -> str:
    """Нормализация слова для сравнения (без пробелов и знаков)."""
    return re.sub(r"[^а-яa-z]", "", _normalize(word))

def _pick_word() -> str:
    """Возвращает случайное слово (нормализованное)."""
    return _normalize_word(random.choice(WORDS_LIST))

def _check_guess(guess: str, target: str) -> list[str]:
    """
    Wordle-алгоритм с корректной обработкой дублирующихся букв.
    Возвращает список эмодзи: 🟩 🟨 ⬛
    """
    n = len(target)
    result       = [MISS] * n
    ...
    for i in range(n):
        if guess_chars[i] == target_chars[i]:
            result[i]       = HIT
            target_chars[i] = None   # занято
            guess_chars[i]  = None   # занято
...
def _format_row(word: str, emojis: list[str]) -> str:
    """Форматирует одну строку попытки: буквы и эмодзи под ними."""
...
def _build_board(session: dict, footer: str = "✍ Отправь предполагаемое слово сообщением.") -> str:
    ...
    history    = session["history"]   # list of (guess, emojis)
...
def register_cipher_handlers(bot: TeleBot) -> None:
    """Вызвать один раз из bot.py: register_cipher_handlers(bot)"""
    ...
    def callback_cipher_start(call: CallbackQuery):
        ...
        SESSIONS[user_id] = {
            ...
            "history":  [],          # [(guess_str, emojis_list), ...]
            "active":   True,
            "chat_id":  chat_id,
            "board_msg_id": None,    # ID сообщения с доской — будем редактировать
        }
    ...
    def handle_cipher_answer(message: Message):
        ...
        # --- Продолжаем игру --- обновляем доску
        board_text = _build_board(session)
        _edit_board(bot, session, board_text)

    # ------------------------------------------------------------------
    # Сообщение когда игрок пишет после завершения сессии (пишет .шифр)
    # ------------------------------------------------------------------
    @bot.message_handler(
        func=lambda m: (
            m.text
            ...
            # Срабатывает только если похоже на ответ (слово из кириллицы)
            and re.fullmatch(r"[а-яёА-ЯЁ]{3,12}", m.text.strip()) is not None
        )
    )
    def handle_cipher_after_game(message: Message):
        # Не мешаем другим хэндлерам — ничего не делаем
        pass
...
def _edit_board(bot: TeleBot, session: dict, text: str) -> None:
    """Редактирует сообщение с игровой доской. При ошибке отправляет новое."""
    try:
