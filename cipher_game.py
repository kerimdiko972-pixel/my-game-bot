...
def reload_data() -> None:
    ...
    # 1. Читаем сырые строки из файла
    raw_wordle = _load_file(WORDLE_FILE)
    
    # 2. Очищаем их (нижний регистр, убираем знаки препинания и дефисы)
    normalized = [_normalize_word(w) for w in raw_wordle if _normalize_word(w)]
    
    # 3. ФИЛЬТР: Оставляем только полноценные слова длиной от 5 до 8 букв.
    # Это автоматически отбросит "ад", "ум", "юг" из начала файла
    # и гигантские слова вроде "самосовершенствование" из конца файла.
    filtered_words = [w for w in normalized if 5 <= len(w) <= 8]

    # Если вдруг файл оказался пуст, используем запасные слова
    if not filtered_words:
        filtered_words = [_normalize_word(w) for w in FALLBACK_WORDS]

    # 4. Присваиваем очищенный список и для загадывания, и для проверки!
    # Теперь бот физически не сможет загадать слово из 2 букв, 
    # и база для проверки будет состоять только из нормальных слов.
...
def _check_guess(guess: str, target: str) -> list[str]:
    ...
    for i in range(n):
        if guess_chars[i] == target_chars[i]:
            result[i]       = HIT
            target_chars[i] = None   # ячейка занята
            guess_chars[i]  = None   # ячейка занята
...
def _build_board(session: dict, footer: str = "✍ Отправь предполагаемое слово сообщением.") -> str:
    ...
    history    = session["history"]   # список кортежей (guess, emojis)
...
def register_cipher_handlers(bot: TeleBot) -> None:
    ...
    def callback_cipher_start(call: CallbackQuery):
        ...
        SESSIONS[user_id] = {
            ...
            "active":   True,
            "chat_id":  chat_id,
            "board_msg_id": None,    # ID сообщения с доской для последующего редактирования
        }
    ...
    def handle_cipher_answer(message: Message):
        ...
        if all(e == HIT for e in emojis):
            ...
            # Финальное обновление игрового поля без текста-подсказки внизу
            board_text = _build_board(session, footer="")
            _edit_board(bot, session, board_text)

            # Выбор случайного типа награды
        ...
        # Игровой процесс продолжается — обновляем основную доску
