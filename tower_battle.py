...
def format_statuses(statuses):
    ...
    for k, v in statuses.items():
        ...
        parts.append(f"{em}×{v}" if v > 1 else em)
    return "  ".join(parts)
...
def apply_weapon_effect(char, state, weapon_key):
    """Проверяет и применяет эффект оружия. Возвращает доп. лог."""
    w = WEAPONS.get(weapon_key)
    if not w or not w.get('effect') or w['effect'] in ('Нет', None):
        return []
    effect = w['effect']
    log = []
    e_st = state.get('enemy_statuses', {})

    if 'Ожог' in effect and random.random() < 0.3:
        e_st['burn'] = max(e_st.get('burn', 0), 2)
        log.append("🔥 Оружие поджигает врага!")
    if 'Слабость' in effect and random.random() < 0.2:
        e_st['weakness'] = max(e_st.get('weakness', 0), 1)
        log.append("⛓️ Оружие накладывает Слабость!")
    if 'Кровотечение' in effect and random.random() < 0.1:
        e_st['bleed'] = max(e_st.get('bleed', 0), 1)
        log.append("🩸 Оружие вызывает Кровотечение!")
    if 'двойного удара' in effect and random.random() < 0.15:
        log.append("✨ Двойной удар!")
        state['_double_strike'] = True
    if 'Яд' in effect and random.random() < 0.15:
        e_st['poison'] = e_st.get('poison', 0) + 1
        log.append("☠️ Оружие отравляет врага!")
    if 'Шок' in effect:
...
def register_tower_battle(bot):
    ...
    def cb_tb_next_floor(call):
        ...
        event_chance = 0.35
        if (floor - last_event_floor >= 2) and random.random() < event_chance:
