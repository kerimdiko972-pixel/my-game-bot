...
def init_roulette_tables(get_conn):
    ...
    ''')
    try:
        c.execute('ALTER TABLE roulette_stats ADD COLUMN IF NOT EXISTS record_round INTEGER DEFAULT 0')
    except Exception:
        pass
    conn.commit()
    conn.close()
...
def update_rrecord(get_conn, user_id, round_num):
    """Обновляет рекорд раунда если текущий выше сохранённого."""
    conn = get_conn()
    c = conn.cursor()
    c.execute('''
        INSERT INTO roulette_stats (user_id, record_round) VALUES (%s, %s)
        ON CONFLICT (user_id) DO UPDATE
        SET record_round = GREATEST(roulette_stats.record_round, EXCLUDED.record_round)
    ''', (user_id, round_num))
    conn.commit()
    conn.close()


def get_rrecord(get_conn, user_id):
    conn = get_conn()
    c = conn.cursor()
    c.execute('SELECT record_round FROM roulette_stats WHERE user_id=%s', (user_id,))
    row = c.fetchone()
    conn.close()
    return row[0] if row else 0


def get_top_roulette_by_record(get_conn, limit=10):
    conn = get_conn()
    c = conn.cursor()
    c.execute('''
        SELECT u.username, r.record_round, r.user_id
        FROM roulette_stats r
        JOIN users u ON u.user_id = r.user_id
        WHERE r.record_round > 0
        ORDER BY r.record_round DESC LIMIT %s
    ''', (limit,))
    rows = c.fetchall()
    conn.close()
    return rows
...
def dealer_turn(state, bot, chat_id, get_conn, user_id):
    ...
    if shoot_self:
        ...
        if not hit:
            send('💨 Холостой...')
            # Если патроны кончились — перезарядка и ход игроку, без доп. хода
            if state['chamber_pos'] >= len(state['chamber']):
                state, reload_msg = reload_chamber(state)
                send(reload_msg)
                # reload_chamber уже ставит turn = 'player'
            elif not state.get('_dealer_extra_turn_used'):
                # Доп. ход дилера только если патроны ещё есть
...
def register_roulette_handlers(bot, get_conn, get_user, add_money, spend_money):
    ...
    def send_main_menu(chat_id, user_id, msg_id=None):
        ...
        record = get_rrecord(get_conn, user_id)
        text = (
            f'— – - ⬜ СПИНШОТ ДУЭЛЬ ⬛ - – —\n\n'
            f'Денег: 💵 {money:,}\n'
            f'Опыта: ⭐ {exp:,}\n'
            f'Побед: 🎖️ {wins}\n'
            f'Рекорд: 🏆 {record} раунд\n\n'
            f'• – – – – – – – – – – – – – – – – – •'
        )
    ...
    def cb_rlt_leaderboard(call):
        user_id = call.from_user.id
        top     = get_top_roulette_by_record(get_conn, 10)
        medals  = ['🥇', '🥈', '🥉']
        text    = '– 🏆 РЕКОРДЫ ТОП 10 ИГРОКОВ 🏆 –\n\n'
        user_place = None

        for i, (uname, record, uid) in enumerate(top, 1):
            medal = medals[i-1] if i <= 3 else f'{i}.'
            text += f'{medal} {uname} – 🏆 {record} раунд\n'
            if uid == user_id:
                user_place = i
        ...
        if user_place is None:
            my_record = get_rrecord(get_conn, user_id)
            conn = get_conn()
            c = conn.cursor()
            c.execute('''
                SELECT COUNT(*) + 1 FROM roulette_stats
                WHERE record_round > COALESCE(
                    (SELECT record_round FROM roulette_stats WHERE user_id=%s), 0)
            ''', (user_id,))
            place = c.fetchone()[0]
            conn.close()
            text += f'\n👤 Ты на {place} месте — рекорд: {my_record} раунд'
            text += '\n• – – – – – – – – – – – – – – – – – •'
    ...
    def cb_rlt_take_money(call):
        ...
        update_rrecord(get_conn, user_id, state['round'])
        clear_rstate(get_conn, user_id)
...
def handle_round_win(bot, chat_id, user_id, state, get_conn, add_money):
    ...
    if mode == 'classic' and rnd >= 3:
        ...
        bot.send_message(chat_id, text, reply_markup=kb)
        update_rrecord(get_conn, user_id, rnd)
        update_rrecord(get_conn, user_id, rnd)
...
def handle_game_loss(bot, chat_id, user_id, state, get_conn, add_money):
    ...
    update_rrecord(get_conn, user_id, rnd)  # сохраняем рекорд даже при поражении
    clear_rstate(get_conn, user_id)
