...
def dealer_turn(state, bot, chat_id, get_conn, user_id):
    """
    Выполняет полный ход дилера — использует предметы и стреляет.
    Все действия сопровождаются сообщениями в чат.
    """
    ...
    # Если дилер скован — пропускает ход
    if state.get('dealer_skip'):
        state['dealer_skip'] = False
        send('⛓️ Дилер скован и пропускает ход')
        return state

    # Определяем текущий патрон (если известен боту)
    chamber = state['chamber']
    pos     = state['chamber_pos']
    current_is_live = chamber[pos] if pos < len(chamber) else None

    p_hp   = state['player_hp']
    d_hp   = state['dealer_hp']
    live   = sum(1 for i, c in enumerate(chamber) if i >= pos and c)
    blank  = sum(1 for i, c in enumerate(chamber) if i >= pos and not c)
    total  = live + blank
    ...
    if '📞 Телефон' in items and total > 1:
        ...
        # Выбрать случайный патрон из оставшихся
    ...
    # ── 3. Контроль / подготовка ──

    # Инвертор — если текущий плохой (холостой и хочет стрелять)
    ...
    # Наручники — если игроку много HP или дилеру плохо
    ...
    # Адреналин — украсть у игрока ценный предмет
    if '💉 Адреналин' in items and p_items:
        ...
        if stolen != '💉 Адреналин':
            ...
            # Сразу использует украденный предмет
            state = dealer_use_stolen(state, stolen, bot, chat_id)
            d_hp = state['dealer_hp']
            p_hp = state['player_hp']
            current_is_live = state['chamber'][state['chamber_pos']] if state['chamber_pos'] < len(state['chamber']) else None

    # Пила — если выгодно стрелять и пила не использована
    if not state.get('saw_active') and '🪚 Пила' in items:
        if (live_chance >= 0.5 or p_hp <= 2) and current_is_live is not False:
            items.remove('🪚 Пила')
            state['saw_active'] = True
            send('🪚 Дилер пилит ствол.\nСледующий боевой выстрел нанесёт 2 урона')

    # Пиво — если текущий точно боевой и стрелять невыгодно
    if current_is_live is True and '🍺 Пиво' in items and d_hp > 2 and live_chance < 0.3:
        items.remove('🍺 Пиво')
        dropped = chamber[pos]
        state['chamber_pos'] += 1
        state['shots_fired'] += 1
    ...
    chamber = state['chamber']
    current = chamber[pos]
    live2  = sum(1 for i, c in enumerate(chamber) if i >= pos and c)
    blank2 = sum(1 for i, c in enumerate(chamber) if i >= pos and not c)
    total2 = live2 + blank2
    live_ch = live2 / total2 if total2 > 0 else 0.5

    # Если пуля точно холостая — стрелять в себя (доп. ход)
    if current_is_live is False or (current_is_live is None and live_ch < 0.3):
        send('Дилер целится на себя. . .', italic=True)
        time.sleep(1.1)
        state, hit, damage = fire(state, target='dealer')
        if not hit:
            send('💨 Холостой...')
            # Дилер получает доп. ход — рекурсивно
